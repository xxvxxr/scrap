...
(async () => {
    ...
    const performNikeSearch = async (searchInputSelector = 'input#VisualSearchInput') => {
        ...
        try {
            ...
            const options = await page.$$eval('div > figure', options => {
    ...
    const googleSearch = async (searchInputSelector = 'textarea[aria-label="Search"]') => {
        const { page, browser, siteUrl } = await setupPuppeteer();
        let search_input = readlineSync.question('What would you like to search for? ');
        ...
        try {
            ...
            await page.waitForNavigation();
            const options = await page.$$eval('* > div:nth-child(9) > div:nth-child(2) > div', options => {
                const mapOFOptions = options.map(option => {
                    const { innerText, innerHTML } = option;
